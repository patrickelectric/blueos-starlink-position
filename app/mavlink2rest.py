...
class Mavlink2RestBase:
    ...
    def __init__(
        self,
        host: str = "http://127.0.0.1/mavlink2rest",
        vehicle: int = 1,
        component: int = 220,
        get_vehicle: int = 1,
        get_component: int = 1,
    ...
    def get(self, path: str):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(
                    f"Got HTTP Error: {response.status_code} {response.reason} {response.text}"
                )
                return None
    ...
    def get_message(
        ...
        message_path = (
            f"/mavlink/vehicles/{vehicle}/components/{component}/messages" + path
        )
    ...
    def post(self, path: str, json: object) -> bool:
        ...
        response = None
        try:
            response = requests.post(full_url, json=json, timeout=3)
            if response.status_code == 200:
    ...
    def ensure_message_frequency(self, message_name: str, frequency: int) -> bool:
        ...
        """
        msg_ids = {"VFR_HUD": 74, "SCALED_PRESSURE2": 137}
        message_name = message_name.upper()

        logger.info(
            f"Trying to set message frequency of {message_name} to {frequency} Hz"
        )

        previous_frequency = self.get_float(
            f"/{message_name}/message_information/frequency"
        )
        ...
        if success:
            logger.info(
                f"Successfully set message frequency of {message_name} to {frequency} Hz, was {previous_frequency} Hz"
            )
        return success
...
class Mavlink2Rest(Mavlink2RestBase):
    def get_depth(self):
    ...
    def send_gps_input(
        self,
        global_locator_position: object,
        acoustic_locator_position: object,
        args: object,
        gps_id: int = 0,
    ):
        """
        ...
        try:
            ...
            fix_valid = (
                global_locator_position is not None
                and (global_locator_position["hdop"] < 20 or args.ignore_gps)
                and (
                    acoustic_locator_position is not None
                    and acoustic_locator_position["position_valid"]
                )
                or args.ignore_acoustic
            )
            ...
            out_json["message"]["hdop"] = (
                65535.0
                if global_locator_position is None
                else global_locator_position["hdop"]
            )
            # 300m as maximum range
            out_json["message"]["horiz_accuracy"] = (
                300
                if acoustic_locator_position is None
                else acoustic_locator_position["std"]
            )
            ...
            if global_locator_position is not None:
                out_json["message"]["lat"] = math.floor(
                    global_locator_position["lat"] * 1e7
                )
                out_json["message"]["lon"] = math.floor(
                    global_locator_position["lon"] * 1e7
                )
                # ignore_gps overrides satellites_visible so that ArduSub will trust the GPS
                out_json["message"]["satellites_visible"] = max(
                    global_locator_position["numsats"], 6 if args.ignore_gps else 0
                )
                ...
                if global_locator_position["orientation"] == -1:
                    ...
                    out_json["message"]["yaw"] = math.floor(
                        global_locator_position["orientation"] * 100
                    )  # default
    ...
    def send_gps_input_simple(self, latitude, longitude, altitude, gps_id: int = 0):
        """
        Forwards the locator(ROV) position to mavproxy's GPSInput module
        """
        out_json = self.get_helper_struct("GPS_INPUT")
        ...
        try:
            ...
            out_json["header"]["component_id"] = self.component
            out_json["message"]["gps_id"] = gps_id
            out_json["message"]["ignore_flags"]["bits"] = 1 | 2 | 3 | 8 | 16 | 32 | 128

            out_json["message"]["fix_type"] = 2  # GPS_FIX_TYPE_2D_FIX

            out_json["message"]["hdop"] = 65535.0
            out_json["message"]["horiz_accuracy"] = 5
            out_json["message"]["vdop"] = 65535.0

            out_json["message"]["lat"] = math.floor(latitude * 1e7)
            out_json["message"]["lon"] = math.floor(longitude * 1e7)
            # ignore_gps overrides satellites_visible so that ArduSub will trust the GPS
            out_json["message"]["satellites_visible"] = 6
            out_json["message"]["yaw"] = 0  # invalid
            out_json["message"]["yaw"] = 36000  # remap 0 -> 360
